...
class LayerAssigner:
    ...
    def assign_layers(self, splats: List["Gaussian"]) -> Dict[int, List["Gaussian"]]:
        ...
        for layer_idx in range(self.n_layers):
            ...
            # Assign depth values with much closer spacing to prevent gaps
            # Use non-linear spacing with more layers clustered near neutral depth (0.6)
            # This ensures better overlap and seamless coverage
            if self.n_layers == 1:
                depth_value = 0.6  # Single layer at neutral depth
            else:
                # Use exponential-like distribution clustered around 0.6
                # Back layers: 0.45-0.58, Front layers: 0.62-0.85
                t = layer_idx / (self.n_layers - 1)  # 0 to 1

                if t < 0.5:
                    # Background layers: smooth curve from 0.45 to 0.58
                    depth_value = 0.45 + (t * 2) * 0.13  # 0.45 to 0.58
                else:
                    # Foreground layers: smooth curve from 0.62 to 0.85
                    depth_value = 0.62 + ((t - 0.5) * 2) * 0.23  # 0.62 to 0.85

            # Apply depth-based scale adjustment for better overlap
            # Background layers (lower depth) get larger splats for better coverage
            # Foreground layers (higher depth) keep normal size for detail
            scale_factor = 1.0
            if depth_value < 0.6:  # Background layers
                # Increase size by up to 40% for background layers
                scale_factor = 1.0 + (0.6 - depth_value) * 1.5  # Up to 1.4x for deepest layer
            elif depth_value > 0.7:  # Far foreground layers
                # Slightly increase size for foreground overlap
                scale_factor = 1.0 + (depth_value - 0.7) * 0.3  # Up to 1.15x for front layer

            for splat in layer_splats:
                splat.depth = depth_value
                # Apply scale factor for better layer overlap
                splat.rx *= scale_factor
                splat.ry *= scale_factor
    ...
    def validate_layers(self, layers: Dict[int, List["Gaussian"]]) -> bool:
        ...
        try:
            ...
            for layer_idx, layer_splats in layers.items():
                # Calculate expected depth using the new algorithm
                if self.n_layers == 1:
                    expected_depth = 0.6
                else:
                    t = layer_idx / (self.n_layers - 1)
                    if t < 0.5:
                        expected_depth = 0.45 + (t * 2) * 0.13
                    else:
                        expected_depth = 0.62 + ((t - 0.5) * 2) * 0.23
