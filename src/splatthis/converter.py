"""Torch-based image-to-Gaussian-splat fitter with direct format emitters."""
...
class PNG2SVGConverter:
    """Fit one Gaussian population and emit any supported target format."""
    ...
    def __init__(
        ...
        quality_profile: str = "max-fidelity",
        ...
        self.requested_optimizer_backend = str(optimizer_backend).strip().lower()
        if self.requested_optimizer_backend not in {"torch", "mlx"}:
            raise ValueError(
                f"Unsupported optimizer backend: {optimizer_backend}. Choose: torch, mlx"
            )
        self.optimizer_backend = self.requested_optimizer_backend
        if self.optimizer_backend == "mlx":
            ...
            if not is_mlx_available():
                reason = (
                    ...
                    if is_mlx_imported()
                    else "the optional MLX package is not installed"
                )
                logger.warning(
                    "MLX requested but %s; falling back to Torch.",
                    reason,
                )
        ...
        logger.info(
            "Initialized PNG2SVG converter: max_splats=%s, stages=%s, device=%s, backend=%s->%s, optimizer=%s->%s, blend=%s, seed=%s, profile=%s, resolution_scale=%.2f, init_random_ratio=%.2f",
            self.max_splats,
            self.stages,
            device,
            self.renderer_backend,
            self.resolved_renderer_backend,
            self.requested_optimizer_backend,
    ...
    def convert(
        ...
        *,
        ...
        email_subject: str = "Gaussian splats, drawn by your mail client",
        email_sender: str = "splatthis@localhost",
        email_recipient: str = "you@example.com",
        email_max_splats: int = 285,
        embed_population: bool = False,
        embed_population_in_pixels: bool = False,
    ) -> str:
        """Fit Gaussian splats once and write one self-contained artifact."""
        input_path = str(input_path)
        output_format = self._normalize_output_format(output_format)
        if embed_population_in_pixels and not embed_population:
            raise ValueError("embed_population_in_pixels requires embed_population")
        if embed_population_in_pixels and not preview_png_path:
            raise ValueError("embed_population_in_pixels requires preview_png_path")
        if (
            embed_population
            and output_format not in {"svg", "pptx"}
            and not preview_png_path
        ):
            raise ValueError(
                "population embedding for this format requires preview_png_path"
            )
        output_path = output_path or str(
            Path(input_path).with_suffix(OUTPUT_SUFFIXES[output_format])
        )
        ...
        manifest: Dict[str, Any] = {
            ...
            "config": {
                ...
                "requested_optimizer_backend": self.requested_optimizer_backend,
                "optimizer_backend": self.optimizer_backend,
                "blend_mode": self.blend_mode,
                "output_format": output_format,
                "embed_population": bool(embed_population),
                "embed_population_in_pixels": bool(embed_population_in_pixels),
        ...
        if structure_enabled:
            ...
            if verbose:
                logger.info(
                    "Using precomputed structure maps for init/densify guidance."
                )
        ...
        if verbose:
            logger.info("Generating %s with %s splats...", output_format, len(splats))
        output_content = self._emit_output(
            splats=splats,
            width=width,
            height=height,
            ...
            email_max_splats=email_max_splats,
            embed_population=embed_population,
        )
        if save_json:
            save_splats_json(splats, str(Path(output_path).with_suffix(".json")))
        ...
        final_loss_fn = L1SSIMLoss(
            **self.loss_weights, color_space=self.loss_color_space
        ).to(self.device)
        ...
        final_metrics = dict(internal_metrics)
        ...
        if preview_png_path:
            render_splats_preview_png(
                ...
                embed_splats=splats if embed_population else None,
                embed_in_pixels=embed_population_in_pixels,
            )
        ...
        manifest["acceptance_metric_source"] = "internal"
    ...
    def _initialize_splats(
        ...
        all_positions = (
            base_positions + seed_positions + random_positions + poisson_positions
        )
    ...
    def _optimize_splats(
        ...
        loss_fn = L1SSIMLoss(**self.loss_weights, color_space=self.loss_color_space).to(
            self.device
        )
    ...
    def _optimize_stage(
        ...
        """Optimize one stage with the selected Torch or MLX backend.
    ...
    def _optimize_stage_mlx(
        ...
        *,
        ...
        loss_name = "oklab-l1-ssim" if self.loss_color_space == "oklab" else "l1-ssim"
        result = optimize_stage_mlx(
            splats=splats,
            target_linear_rgb=target.detach().cpu().numpy(),
            width=width,
            height=height,
            num_iters=num_iters,
            config=MlxStageConfig(
                renderer=MlxRendererConfig(
                    ...
                    background_color=tuple(
                        float(value) for value in self._background_linear_rgb[:3]
                    ),
                    compositing_space=self.compositing_space,
                ),
                loss=MlxLossConfig(
                    ...
                    gradient_weight=float(
                        self.loss_weights.get("gradient_weight", 0.0)
                    ),
                ),
                ...
                progress_interval=50 if verbose else 0,
            ),
            learning_rates=self.learning_rates,
            verbose=verbose,
        )
        rendered = torch.from_numpy(
            np.asarray(result.rendered_linear_rgb, dtype=np.float32)
        ).to(device=self.device, dtype=target.dtype)
    ...
    def _run_residual_detail_passes(
        ...
        for pass_idx in range(passes):
            ...
            for idx, (x, y) in enumerate(zip(x_indices, y_indices)):
                ...
                splat.importance = float(
                    np.clip(0.65 + 0.35 * sample_weights[idx], 0.0, 1.0)
                )
            ...
            if verbose:
                logger.info(
                    "Residual detail pass %s: adding %s small splats",
                    pass_idx + 1,
                    len(new_splats),
                )
    ...
    def _normalize_output_format(value: str) -> str:
        ...
        if normalized not in aliases:
            supported = ", ".join(OUTPUT_SUFFIXES)
            raise ValueError(f"Unsupported output format: {value}. Choose: {supported}")
    ...
    def _emit_output(
        ...
        *,
        ...
        email_sender: str,
        email_recipient: str,
        email_max_splats: int,
        embed_population: bool,
    ) -> str:
        """Emit from the fitted population without invoking another pipeline."""
        if output_format == "svg":
            content = self._generate_svg(
                splats, width, height, embed_population=embed_population
            )
            save_svg(
                splats,
                width,
        ...
        if output_format == "canvas":
            from .browser_export import generate_native_canvas_html
        ...
        from .email_export import save_css_email
...
# Clear name for new code; the historical name remains a compatible alias.
SplatConverter = PNG2SVGConverter
